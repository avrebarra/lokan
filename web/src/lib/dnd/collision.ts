...
// source: kan/apps/web/src/views/board/dnd/collision.ts (72 LOC)
// preserved under AGPLv3 — see NOTICE.md
...
function getData(entity: { data: { current: unknown } } | undefined): DragData | undefined {
    ...
    if (overId != null) {
      ...
      if (overData?.type === 'LIST_BODY') {
        const cardContainers = args.droppableContainers.filter((container) => {
          ...
          return data?.type === 'CARD' && data.listPublicId === overData.listPublicId
        })
